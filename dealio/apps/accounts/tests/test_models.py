...
from dealio.apps.accounts.models import SocialAccount, SocialAuthProvider, TokenBlacklist
...
class AccountModelTests(TestCase):
    ...
    def test_token_blacklist_string_contains_username(self):
        user = UserFactory.create(username="amir")
        token = TokenBlacklist.objects.create(user=user, token="access", refresh="refresh")

        self.assertEqual(str(token), "TokenBlacklist for User: amir")
