(() => {
    ...
    function createAmbient() {
        if ($('.devixa-ambient')) return;
        const ambient = document.createElement('div');
        ambient.className = 'devixa-ambient';
    ...
    function createPreloader() {
        if (prefersReducedMotion || $('.devixa-preloader')) return;
        document.body.classList.add('page-is-loading');
        const preloader = document.createElement('section');
        preloader.className = 'devixa-preloader';
        preloader.setAttribute('aria-live', 'polite');
        preloader.innerHTML = `
            <div class="devixa-preloader__card">
                <div class="devixa-preloader__logo">D</div>
                <h2 class="devixa-preloader__title">Devixa Experience</h2>
                <p class="subtitle" style="width:100%;font-size:14px">در حال آماده‌سازی تجربه‌ای نرم، سریع و حرفه‌ای...</p>
                <div class="devixa-preloader__bar"><span></span></div>
            </div>`;
    ...
    function showToast(message) {
        let toast = $('.devixa-toast');
        if (!toast) {
            toast = document.createElement('div');
            toast.className = 'devixa-toast';
