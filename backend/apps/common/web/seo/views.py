...
class SeoSitemapView(View):
    sitemap_logic_class = SeoSitemapLogic

    def get(self, request, *args, **kwargs):
        content = render_to_string(
            SeoTemplateVO.SITEMAP.value,
            {
                "urls": self.sitemap_logic_class().build(
                    request,
                    get_request_project_context(request),
                )
            },
        )
...
class RobotsTxtView(View):
    def get(self, request, *args, **kwargs):
        ...
        sitemap_url = SeoRequestUrlAdapter.from_project(
            request, project
        ).absolute_url(reverse(SeoRouteVO.SITEMAP.value))
        admin_path = (
            "/"
            + os.environ.get("ADMIN_PANEL_URL", "admin/").strip("/")
            + "/"
        )
        disallowed_paths = tuple(
            dict.fromkeys(
                (admin_path, *(path.value for path in SeoRobotsDisallowPathVO))
            )
        )
