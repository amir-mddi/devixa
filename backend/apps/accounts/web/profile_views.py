...
class ProfileDashboardView(AsyncWebViewMixin, LoginRequiredMixin, ProfilePanelContextMixin, TemplateView):
    template_name = AccountWebTemplateVO.PROFILE.value
...
class ProfileMessengerDisconnectView(AsyncWebViewMixin, 
    LoginRequiredMixin, ProfilePanelContextMixin, View
):
    messenger_profile_logic_class = MessengerProfileLogic

    async def post(self, request, profile_id, *args, **kwargs):
        return await sync_to_async(self._sync_post, thread_sensitive=True)(request, profile_id, *args, **kwargs)

    def _sync_post(self, request, profile_id, *args, **kwargs):
        result = self.messenger_profile_logic_class().disconnect(DisconnectMessengerProfileDTO(profile_id=profile_id, user_id=request.user.id))
        message = AccountProfileMessageVO.MESSENGER_DISCONNECTED.value if result.is_success else AccountProfileMessageVO.MESSENGER_DISCONNECT_FAILED.value
...
class ProfileIdentityUpdateView(AsyncWebViewMixin, LoginRequiredMixin, ProfilePanelContextMixin, View):
    ...
    def _sync_post(self, request, *args, **kwargs):
        ...
        if not form.is_valid():
            return self.render_panel(active_section=AccountProfileSectionVO.PROFILE.value, profile_form=form, status=400)
        result = self.profile_logic_class().update_profile(form.to_dto(user_id=str(request.user.id)))
        if not result.is_success:
            form.add_error(AccountWebProfileErrorPresenter.field_for(result.error_code), AccountWebProfileErrorPresenter.message_for(result.error_code))
            return self.render_panel(active_section=AccountProfileSectionVO.PROFILE.value, profile_form=form, status=400)
...
class ProfileContactUpdateView(AsyncWebViewMixin, LoginRequiredMixin, ProfilePanelContextMixin, View):
    profile_logic_class = AccountProfileLogic
    ...
    def _sync_post(self, request, *args, **kwargs):
        ...
        if not form.is_valid():
            return self.render_panel(active_section=AccountProfileSectionVO.CONTACT.value, contact_form=form, status=400)
        result = self.profile_logic_class().update_contacts(form.to_dto(user_id=str(request.user.id)))
        if not result.is_success:
            form.add_error(AccountWebProfileErrorPresenter.field_for(result.error_code), AccountWebProfileErrorPresenter.message_for(result.error_code))
            return self.render_panel(active_section=AccountProfileSectionVO.CONTACT.value, contact_form=form, status=400)
...
class ProfileEmailVerificationSendView(AsyncWebViewMixin, 
    LoginRequiredMixin, ProfilePanelContextMixin, View
):
    ...
    def _sync_post(self, request, *args, **kwargs):
        ...
        if not user.email:
            ...
            try:
                ...
                logger.exception('Sending a profile email verification code failed for user %s.', user.id)
                messages.error(request, AccountProfileMessageVO.EMAIL_SEND_FAILED.value)
            else:
                message = AccountProfileMessageVO.EMAIL_CODE_SENT.value if issued else AccountProfileMessageVO.EMAIL_CODE_STILL_ACTIVE.value
...
class ProfileEmailVerificationView(AsyncWebViewMixin, LoginRequiredMixin, ProfilePanelContextMixin, View):
    account_logic_class = AccountLogicRepository
    ...
    def _sync_post(self, request, *args, **kwargs):
        ...
        if not form.is_valid():
            return self.render_panel(active_section=AccountProfileSectionVO.CONTACT.value, email_code_form=form, status=400)
        if request.user.email_verified:
            messages.info(request, AccountProfileMessageVO.EMAIL_ALREADY_VERIFIED.value)
        elif self.account_logic_class().check_email_validation_code(request.user, form.cleaned_data['code']):
...
class ProfilePhoneVerificationSendView(AsyncWebViewMixin, 
    LoginRequiredMixin, ProfilePanelContextMixin, View
):
    account_logic_class = AccountLogicRepository
    ...
    def _sync_post(self, request, *args, **kwargs):
        result = self.account_logic_class().send_phone_verification_code(SendPhoneVerificationCodeDTO(user_id=str(request.user.id)))
        if result.is_success:
            message = AccountPhoneVerificationApiMessageVO.CODE_SENT.value if result.code_issued else AccountPhoneVerificationApiMessageVO.CODE_STILL_ACTIVE.value
            messages.success(request, message)
        else:
            messages.error(request, AccountWebProfileErrorPresenter.phone_verification_message_for(result.error_code))
        return self.profile_redirect(AccountProfileSectionVO.CONTACT)
...
class ProfilePhoneVerificationView(AsyncWebViewMixin, LoginRequiredMixin, ProfilePanelContextMixin, View):
    account_logic_class = AccountLogicRepository
    ...
    def _sync_post(self, request, *args, **kwargs):
        ...
        if not form.is_valid():
            return self.render_panel(active_section=AccountProfileSectionVO.CONTACT.value, phone_code_form=form, status=400)
        result = self.account_logic_class().verify_phone_number(VerifyPhoneNumberDTO(user_id=str(request.user.id), code=form.cleaned_data['code']))
        if result.is_success:
            messages.success(request, AccountPhoneVerificationApiMessageVO.VERIFIED.value)
            return self.profile_redirect(AccountProfileSectionVO.CONTACT)
        form.add_error('code', AccountWebProfileErrorPresenter.phone_verification_message_for(result.error_code))
        return self.render_panel(active_section=AccountProfileSectionVO.CONTACT.value, phone_code_form=form, status=400)
...
class ProfileTicketCreateView(AsyncWebViewMixin, LoginRequiredMixin, ProfilePanelContextMixin, View):
    ...
    def _sync_post(self, request, *args, **kwargs):
        ...
        if not form.is_valid():
            return self.render_panel(active_section=AccountProfileSectionVO.TICKETS.value, ticket_form=form, status=400)
        try:
            self.support_logic_class().create_account_ticket(user=request.user, subject=form.cleaned_data['subject'], message=form.cleaned_data['message'])
        except APIException:
            messages.error(request, AccountProfileMessageVO.SUPPORT_OPERATION_FAILED.value)
        else:
            messages.success(request, AccountProfileMessageVO.SUPPORT_TICKET_CREATED.value)
...
class ProfileTicketReplyView(AsyncWebViewMixin, LoginRequiredMixin, ProfilePanelContextMixin, View):
    support_logic_class = BotSupportLogicRepository

    async def post(self, request, ticket_id, *args, **kwargs):
        return await sync_to_async(self._sync_post, thread_sensitive=True)(request, ticket_id, *args, **kwargs)

    def _sync_post(self, request, ticket_id, *args, **kwargs):
        form = SupportReplyTemplateForm(request.POST)
        if not form.is_valid():
            messages.error(request, AccountProfileMessageVO.SUPPORT_OPERATION_FAILED.value)
            return self.profile_redirect(AccountProfileSectionVO.TICKETS)
        try:
            self.support_logic_class().add_account_user_message(ticket_id=ticket_id, user=request.user, message=form.cleaned_data['message'])
        except APIException:
            messages.error(request, AccountProfileMessageVO.SUPPORT_OPERATION_FAILED.value)
        else:
...
class ProfileCourseReviewView(AsyncWebViewMixin, LoginRequiredMixin, ProfilePanelContextMixin, View):
    course_logic_class = CourseLogicRepository

    async def post(self, request, course_id, *args, **kwargs):
        return await sync_to_async(self._sync_post, thread_sensitive=True)(request, course_id, *args, **kwargs)

    def _sync_post(self, request, course_id, *args, **kwargs):
        form = CourseReviewTemplateForm(request.POST)
        if not form.is_valid():
            messages.error(request, AccountProfileMessageVO.REVIEW_OPERATION_FAILED.value)
            return self.profile_redirect(AccountProfileSectionVO.COURSES)
        try:
            self.course_logic_class().submit_review(user=request.user, dto=form.to_dto(course_id=course_id))
        except APIException:
            messages.error(request, AccountProfileMessageVO.REVIEW_OPERATION_FAILED.value)
        else:
...
class ProfilePaymentReceiptUploadView(AsyncWebViewMixin, 
    LoginRequiredMixin, ProfilePanelContextMixin, View
):
    ...
    def _sync_post(self, request, payment_id, *args, **kwargs):
        form = PaymentReceiptUploadTemplateForm(request.POST, request.FILES, prefix=f'receipt-{payment_id}')
        if not form.is_valid():
            return self.render_panel(active_section=AccountProfileSectionVO.BILLING.value, receipt_upload_forms={str(payment_id): form}, status=400)
        try:
            self.billing_logic_class().upload_receipt(user=request.user, dto=form.to_dto(payment_id=payment_id))
        except APIException:
            form.add_error(None, AccountProfileMessageVO.PAYMENT_RECEIPT_UPLOAD_FAILED.value)
            return self.render_panel(active_section=AccountProfileSectionVO.BILLING.value, receipt_upload_forms={str(payment_id): form}, status=400)
        messages.success(request, AccountProfileMessageVO.PAYMENT_RECEIPT_UPLOADED.value)
